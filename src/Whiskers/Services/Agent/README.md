...
The **acting agent**: you give it an operations task in natural language and it plans and executes using Whiskers's own MCP tools. It is provider-agnostic (OpenAI, OpenRouter, Ollama, Gemini, Anthropic, and Claude Code) and bounded by inescapable [guardrails](Guardrails/) enforced at the tool-execution boundary, never in the prompt.
...
| `AgentToolRegistry.cs` | Derives the agent's tool set from the **enabled** modules' `[McpServerToolType]` classes (via `IModuleRegistry`, RoadToSAP §2.3 — so a disabled module's tools are invisible to the agent, not just unregistered at the MCP boundary), reflects their `[McpServerTool]` methods into LLM function schemas, cross-checks against the canonical permission levels, and excludes agent-disallowed tools (e.g. `instruct_agent`). |
...
| `ClaudeCodeRuntime.cs` | Orchestrates the Claude Code CLI as a subprocess, configured via `--mcp-config` to point back at Whiskers's `/mcp` endpoint so its tool calls run through the same guardrail gate. |
...
- Tests: [`../../../Whiskers.Tests/`](../../../Whiskers.Tests/) (`AgentService*`, `AgentSession`, `AgentToolInvoker`, `AgentToolRegistry`, `ClaudeCodeOutputParser`, ...)
