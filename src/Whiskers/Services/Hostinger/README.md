...
Like Hetzner, credentials are **per Whiskers server**, so the bearer token is supplied per call. List/get responses are parsed tolerantly, since Hostinger's payload shapes vary.
