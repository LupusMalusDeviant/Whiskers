...
/// <summary>
/// Provider-agnostic cloud control. Resolves a Whiskers server's configured provider + per-server
/// API key, finds the matching VM/server in that account by public IP, and dispatches power/snapshot/
/// metric operations to the right provider client. Callers work in terms of Whiskers servers and
...
public class CloudControlService : ICloudControlService
{
    ...
    // Match a Whiskers server to a VM in a PRE-FETCHED account listing (public IP, then name) and
    ...
    private static CloudServerInfo? MapHetzner(Whiskers.Models.ServerConfig sw, List<HetznerServer> servers)
    {
        var ip = HostOf(sw);
    ...
    private static CloudServerInfo? MapHostinger(Whiskers.Models.ServerConfig sw, List<HostingerVm> vms)
    {
        var ip = HostOf(sw);
    ...
    {
        // Group by (provider, token) so an account with N Whiskers servers is listed ONCE instead of
    ...
    private async Task<(Whiskers.Models.ServerConfig sw, CloudServerInfo info)> ContextAsync(string idOrName)
    {
        var sw = ResolveWhiskers(idOrName)
