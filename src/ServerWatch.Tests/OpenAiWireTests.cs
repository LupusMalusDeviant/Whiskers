...
public class AgentProviderFactoryTests
{
    ...
    {
        var provider = Factory().Resolve(new ServerWatch.Configuration.AgentSettings { Provider = id });
    ...
    public void Unknown_provider_throws()
    {
        Assert.Throws<NotSupportedException>(() =>
            Factory().Resolve(new ServerWatch.Configuration.AgentSettings { Provider = "does-not-exist" }));
    }
    ...
    {
        var provider = Factory().Resolve(new ServerWatch.Configuration.AgentSettings { Provider = "ollama", Endpoint = "" });
