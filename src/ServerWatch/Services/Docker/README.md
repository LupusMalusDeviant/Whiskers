...
| `IDockerConnectionManager.cs` / `DockerConnectionManager.cs` | Provides and caches a `DockerClient` per server, with self-healing reconnect for dead SSH tunnels. Also builds each client (local / SSH-tunnel / TCP+mTLS transport, client certs + CA trust for mTLS). |
