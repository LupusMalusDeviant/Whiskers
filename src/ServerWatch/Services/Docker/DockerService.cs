...
public class DockerService : IDockerService
{
    ...
    public async Task<(string StdOut, string StdErr, int ExitCode)> ExecInContainerAsync(string containerId, string[] command, string? serverId = null, TimeSpan? timeout = null)
    {
        var client = await GetClient(serverId);
        var execParams = new global::Docker.DotNet.Models.ContainerExecCreateParameters
        {
            Cmd = command,
            AttachStdout = true,
            AttachStderr = true
        ...
        // Read with size limit to prevent "Stream was too long" errors
        const int maxBytes = 5 * 1024 * 1024; // 5MB max output
        ...
        try
        {
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                var target = result.Target == MultiplexedStream.TargetStream.StandardOut ? stdoutBuf : stderrBuf;
                if (target.Length + result.Count <= maxBytes)
                {
        ...
        if (truncated)
            stdout += "\n... (Ausgabe gekürzt — verwende LIMIT für kleinere Ergebnisse)";

        // Get exit code
        var inspect = await client.Exec.InspectContainerExecAsync(exec.ID);
        return (stdout, stderr, (int)inspect.ExitCode);
    }

    // === Resource Limits ===

    public async Task<(long NanoCpus, long MemoryBytes, long MemoryReservation)> GetContainerResourceLimitsAsync(string containerId, string? serverId = null)
    {
        var client = await GetClient(serverId);
        var inspect = await client.Containers.InspectContainerAsync(containerId);
        return (
    ...
    public async Task UpdateContainerResourcesAsync(string containerId, double? cpuLimit, long? memoryLimitBytes, string? serverId = null)
    {
        var client = await GetClient(serverId);
        var updateParams = new global::Docker.DotNet.Models.ContainerUpdateParameters();

        if (cpuLimit.HasValue)
            updateParams.NanoCPUs = (long)(cpuLimit.Value * 1e9);
        if (memoryLimitBytes.HasValue)
            updateParams.Memory = memoryLimitBytes.Value;

        await client.Containers.UpdateContainerAsync(containerId, updateParams);
    }
