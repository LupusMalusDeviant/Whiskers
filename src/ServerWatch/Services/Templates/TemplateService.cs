...
public class TemplateService : ITemplateService
{
    ...
    private static readonly List<AppTemplate> Templates = new()
    {
        ...
        T("n8n", "n8n", "Workflow-Automatisierung", "Automatisierung", "AccountTree",
            "services:\n  n8n:\n    image: n8nio/n8n:latest\n    container_name: {PROJECT}-n8n\n    environment:\n      # n8n 1.x removed N8N_BASIC_AUTH_*; create the owner account in the UI on first launch.\n      # N8N_ENCRYPTION_KEY pins credential encryption so saved credentials survive restarts.\n      - N8N_ENCRYPTION_KEY={N8N_ENCRYPTION_KEY}\n    volumes:\n      - n8n_data:/home/node/.n8n\n    ports:\n      - \"{PORT}:5678\"\n    restart: unless-stopped\nvolumes:\n  n8n_data:",
            new() { ["PORT"] = "5678" }, new() { "N8N_ENCRYPTION_KEY" }),
        ...
        T("rocketchat", "Rocket.Chat", "Team-Chat Plattform", "Kommunikation", "Forum",
            "services:\n  rocketchat:\n    image: rocket.chat:latest\n    container_name: {PROJECT}-rocketchat\n    environment:\n      - MONGO_URL=mongodb://mongo:27017/rocketchat?replicaSet=rs0\n      - MONGO_OPLOG_URL=mongodb://mongo:27017/local?replicaSet=rs0\n      - ROOT_URL=http://localhost:{PORT}\n    ports:\n      - \"{PORT}:3000\"\n    depends_on:\n      - mongo\n    restart: unless-stopped\n  # Rocket.Chat requires MongoDB as a replica set (oplog for realtime) or it won't start; bitnami\n  # auto-initialises a single-node rs0. Mongo is internal-only, so an empty password is acceptable.\n  mongo:\n    image: bitnami/mongodb:6.0\n    container_name: {PROJECT}-rc-mongo\n    environment:\n      - MONGODB_REPLICA_SET_MODE=primary\n      - MONGODB_REPLICA_SET_NAME=rs0\n      - MONGODB_ADVERTISED_HOSTNAME=mongo\n      - ALLOW_EMPTY_PASSWORD=yes\n    volumes:\n      - rc_mongo:/bitnami/mongodb\n    restart: unless-stopped\nvolumes:\n  rc_mongo:",
            new() { ["PORT"] = "3004" }, new()),
