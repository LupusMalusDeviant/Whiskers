...
All of ServerWatch's business logic lives here. Each subfolder is one cohesive capability, defined **interface-first** (`IFoo` + `Foo`) and registered in [`Program.cs`](../Program.cs). UI components ([`../Components`](../Components)) and MCP tools ([`../Mcp`](../Mcp)) depend on these interfaces.
