...
**Identity & failure handling (2026-07):** a finding's identity key is built from the container **name** (not its ID), so a container recreate (every image update changes the ID) keeps the persisted age and does not re-notify every CVE. A failed scan (Trivy timeout, transient apt error) returns an empty result with `Error` set; the monitor **keeps the previous good results** instead of overwriting them, avoiding a false "clean" state and a re-notification storm on the next successful scan.
