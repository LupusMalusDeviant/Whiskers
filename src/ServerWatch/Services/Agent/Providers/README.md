...
Provider-agnostic LLM access with tool calling. One implementation per **wire format**; each translates ServerWatch's provider-neutral types ([`../../../Models/Agent/AgentDtos.cs`](../../../Models/Agent/AgentDtos.cs)) into a provider's request/stream shape and back, so the idiosyncrasies never leak out of the implementation.
