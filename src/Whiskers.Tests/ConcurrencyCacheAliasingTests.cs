...
// Regression + concurrency tests for the cache-aliasing / read-lock fixes (Bean Whiskers-sdn3):
...
public class ConcurrencyCacheAliasingTests
{
    ...
    private static Whiskers.Services.ServerConfig.ServerConfigService NewServerConfig()
        => new(
            Options.Create(new DockerSettings()),
            NullLogger<Whiskers.Services.ServerConfig.ServerConfigService>.Instance,
            TempStore());
