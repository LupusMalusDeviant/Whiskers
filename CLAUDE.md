...
- **Einstiegspunkte:** `src/ServerWatch/Program.cs` — Composition-Root: DI, Middleware, MCP-Server-Registrierung, OAuth/OIDC, SignalR-Hubs, Webhook-Endpunkte.
- **Struktur:** `src/ServerWatch/` mit Components (Blazor UI), Mcp + Mcp/Tools (MCP-Auth + Tool-Defs), Services (Docker/Auth/Server/Vault/Agent/Mcp/Deployment/Metrics/Terminal/CVE/Scheduler), Hubs (SignalR), Configuration (typed settings), Models, Utils (SecretRedactor, ShellQuoting); `src/ServerWatch.Tests/` (xUnit); `deploy/telemetry/` (mTLS-Templates); `docs/`.
- **Tests:** xUnit in `src/ServerWatch.Tests/`, run: `dotnet test src/ServerWatch.Tests/ServerWatch.Tests.csproj`
- **Build:** `dotnet build src/ServerWatch/ServerWatch.csproj` → `bin/Release/net10.0` (Docker: `/app/publish`)
...
- **Artifact-Backend:** beans (prefix: `ServerWatch-`)
